...
bool Zombie::getAttacked(int dir)  // return true if dies
{
    ...
    //This step has moved the zombie towards the direction
    else if(m_health==1){
...
void Arena::display() const
{
    ...
    for (r = 0; r < rows(); r++)
        for (c = 0; c < cols(); c++){
            int nZombiesHere=numZombiesAt(r+1, c+1); //?
            if(nZombiesHere==1)
                grid[r][c]='Z';
            else if(nZombiesHere>=2 && nZombiesHere<=8)
                grid[r][c]='0'+nZombiesHere;
            else if(nZombiesHere>=9)
                grid[r][c]='9';
        }
...
bool Arena::attackZombieAt(int r, int c, int dir)
{
    ...
    for(int i=0;i<m_nZombies;i++){
        if(m_zombies[i]->row()==r && m_zombies[i]->col()==c){
            ...
            if(m_zombies[i]->getAttacked(dir)) //The position of the zombie has already been altered
            {
                delete m_zombies[i];
            ...
            //At most one zombie can be injured, so break out of the loop and continue
            else
                break;
        }
    }
        //No zombie is injured
        return false;
}
...
bool Arena::moveZombies()
{
    ...
    {
        m_zombies[k]->move();
        if(m_player->row()==m_zombies[k]->row()
           && m_player->col() == m_zombies[k]->col())
            m_player->setDead();
    }
